...
public class TriviaAnonModel : PageModel
{
    private readonly ILogger<TriviaAnonModel> _logger;
    private readonly IHttpGetCallService _service;
    private readonly TriviaSparkWebContext dbContext;
    private Match? sessionMatch;

    public TriviaAnonModel(ILogger<TriviaAnonModel> logger,
    ...
    private async Task<Match> GetMoreQuestions(CancellationToken ct)
    {
        ...
        foreach (var question in source.Questions)
        {
            ...
            var existingQuestion = dbContext.Questions.Find(question.QuestionId);
            ...
            var matchQuestion = sessionMatch.MatchQuestions.Where(w => w.QuestionId == question.QuestionId).FirstOrDefault();
