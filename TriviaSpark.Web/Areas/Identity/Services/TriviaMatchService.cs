...
{
    private readonly Core.Match.Entities.TriviaSparkWebContext _db;
    ...
    Core.Match.Entities.TriviaSparkWebContext triviaSparkWebContext) : base(new MatchModel())
    {
        _logger = logger;
    ...
    private Core.Match.Entities.Question Create(QuestionModel question)
    {
        Core.Match.Entities.Question results = new();
        try
        {
            List<Core.Match.Entities.QuestionAnswer> answers = new()
            {
                new Core.Match.Entities.QuestionAnswer()
                {
            ...
            {
                answers.Add(new Core.Match.Entities.QuestionAnswer()
                {
                    AnswerText = answer,
                    IsCorrect = false,
                    IsValid = true,
            ...
            return new Core.Match.Entities.Question()
            {
    ...
    private MatchModel? Create(Core.Match.Entities.Match? dbMatch, Core.Match.Entities.QuestionAnswer? dbAnswer = null)
    {
        if (dbMatch is null)
        {
            return default;
    ...
    private static QuestionModel Create(Core.Match.Entities.MatchQuestion matchQuestion)
    {
    ...
    private static new Core.Match.Entities.Match CreateMatch()
    {
        return new Core.Match.Entities.Match()
        {
            UserId = "66eae7b7-c163-4913-8aaf-421a23f0d5d9",
            MatchQuestions = new List<Core.Match.Entities.MatchQuestion>(),
            MatchQuestionAnswers = new List<Core.Match.Entities.MatchQuestionAnswer>(),
    ...
    private async Task<Core.Match.Entities.Match> GetMatchAsync(int MatchId, CancellationToken ct = default)
    {
    ...
    private async Task<Core.Match.Entities.QuestionAnswer?> SetMatchAnswer(int matchId, QuestionAnswerModel currentAnswer, CancellationToken ct)
    {
        try
        {
            ...
            if (dbMatchAnswer is null)
            {
                ...
                _db.MatchAnswers.Add(new Core.Match.Entities.MatchQuestionAnswer()
                {
    ...
    public override async Task<MatchModel?> AddAnswerAsync(int matchId, QuestionAnswerModel? currentAnswer, CancellationToken ct = default)
    {
        ...
        try
        {
            Core.Match.Entities.QuestionAnswer? dbAnswer = await SetMatchAnswer(matchId, currentAnswer, ct);
            Core.Match.Entities.Match match = await GetMatchAsync(matchId, ct);
    ...
    public override async Task<MatchModel?> GetMoreQuestionsAsync(int MatchId, int NumberOfQuestionsToAdd = 1, Difficulty difficulty = Difficulty.Easy, CancellationToken ct = default)
    {
        try
        {
            ...
            foreach (var question in newQuestions)
            {
                ...
                {
                    Core.Match.Entities.Question dbQuestion = Create(question);
                ...
                if (matchQuestion is null)
                {
                    matchQuestion = new Core.Match.Entities.MatchQuestion()
                    {
                        MatchId = MatchId,
                        QuestionId = question.QuestionId,
                    };
    ...
    public override async Task<List<MatchModel>> GetUserMatchesAsync(ClaimsPrincipal user, int? MatchId = null, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            foreach (Core.Match.Entities.Match match in userMatches)
            {
    ...
    public override async Task<MatchModel> CreateMatchAsync(MatchModel newMatch, ClaimsPrincipal user, CancellationToken ct = default)
    {
        ...
        var currentUserId = await _db.Users
            ...
            .AsNoTracking()
            .FirstOrDefaultAsync(cancellationToken: ct);
        Core.Match.Entities.Match match = Create(newMatch, currentUserId);
    ...
    private Core.Match.Entities.Match Create(MatchModel newMatch, string? currentUserId)
    {
        return new Core.Match.Entities.Match()
        {
            MatchQuestions = new List<Core.Match.Entities.MatchQuestion>(),
    ...
    public override async Task<MatchModel?> GetUserMatchAsync(ClaimsPrincipal user, int? MatchId = null, CancellationToken ct = default)
    {
        try
        {
            ...
            Core.Match.Entities.Match? match;
