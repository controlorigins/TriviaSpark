...
{
    public class TriviaMatchService : IDisposable, ITriviaMatchService
    {
        ...
        public async Task<Match> CreateMatchAsync(string userId, string matchName)
        {
            var results = new Match()
            {
                UserId = userId,
                MatchQuestions = new List<MatchQuestion>(),
                MatchQuestionAnswers = new List<MatchQuestionAnswer>(),
            ...
            dbContext.Matches.Add(results);
            await dbContext.SaveChangesAsync();
            return results;
        }
        public async Task<Match> GetMoreQuestions(CancellationToken ct)
        {
        ...
        public async Task<Question> GetQuestionAsync(CancellationToken ct = default)
        {
            sessionMatch = GetTriviaMatch();
            if (sessionMatch.MatchQuestions.Count == 0 || sessionMatch.IsMatchFinished())
            {
                sessionMatch = await GetMoreQuestions(ct);
            }
            return sessionMatch.GetRandomTrivia() ?? new Question() { };
        }

        public async Task<MatchQuestionAnswer?> SaveQuestionAnswerAsync(QuestionAnswer TheAnswer)
        {
            sessionMatch = GetTriviaMatch();
            try
            {
                var TheTrivia = sessionMatch.MatchQuestions.FirstOrDefault(w => w.QuestionId == TheAnswer.QuestionId).Question;
                TheAnswer.QuestionId = TheTrivia.QuestionId;
                TheAnswer.Question = TheTrivia;
                var result = sessionMatch.AddAnswer(TheAnswer);
                await dbContext.SaveChangesAsync();
                return result;
            }
            catch (Exception ex)
